...
class p {
  //MARK: コンストラクタ
  constructor(t, s, i, n, h, r, a) {
    this.cfg = t, this.hTag = s, this.main = i, this.val = n, this.prpPrs = h, this.sndMng = r, this.sys = a, s.let_ml = (e) => this.#Z(e), s.endlet_ml = () => !1, s.dump_stack = () => this.#tt(), s.dump_script = (e) => this.#st(e), s.else = // その他ifブロック開始
    ...
    switch (this.#h) {
      ...
      default:
          ...
          return this.#h = 2, this.main.setLoop(!1, `関数 ${t} ブレーク`), this.sys.callHook("stopOnBreakpoint", {}), this.sys.send2Dbg("stopOnBreakpoint", {}), !0;
        {
          ...
          const n = this.#h === 0;
          this.#h = 2, this.main.setLoop(!1, n ? (
            // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
            (i.condition ? "条件" : "ヒットカウント") + "ブレーク"
          ) : "ステップ実行");
          const h = n ? "stopOnBreakpoint" : "stopOnStep";
  ...
  #J() {
    const t = this.#h === 3 ? 1 : 0, s = this.#s.aToken[this.#t - 1 + t], i = this.#_(this.#i), n = T(s), h = n ? `[${n}]` : s, r = String(this.val.getVal("mp:const.sn.macro") ?? "{}");
    if (this.#t === 0) return [{ fn: i, ln: 1, col: 1, nm: h, ma: r }];
    const a = this.#c(this.#s, this.#t), l = [{ fn: i, ln: a.ln, col: a.col_s + 1, nm: h, ma: r }], e = this.#n.length;
    if (e === 0) return l;
    for (let c = e - 1; c >= 0; --c) {
  ...
  async タグ解析(t, s) {
    ...
    for (const [e, { val: c, def: f }] of Object.entries(n)) {
      ...
      f !== void 0 && (o = this.prpPrs.getValAmpersand(f), o !== "undefined" && (h[e] = o));
    }
    ...
    const l = this.#Q[t];
    return l && d(h, "canskip", this.#Y[t] ?? !0) && this.#x.isSkipping ? l(n) : i(h);
  }
  // 文字出現演出を待つタグ
  ...
  #tt() {
    ...
    if (i > 0) {
      console.info(s);
      for (let n = i - 1; n >= 0; --n) {
        const h = this.#n[n], r = h.csArg[":hMp"], a = r ? r[":タグ名"] : void 0, l = h.csArg[":タグ名"] ?? "", e = this.#c(this.#a[h.fn], h.idx);
        console.info(
          `${String(i - n)}つ前のコール元 fn:${h.fn} line:${String(e.ln)} col:${String(e.col_s + 1)}${a ? "（[" + a + "]マクロ内）" : " "}で [${l} ...]をコール`
        );
      }
    }
...
`).slice(-this.#I), i = s.length;
    ...
    const n = String(this.#e).length, h = this.#c(this.#s, this.#t);
    for (let r = 0; r < i; ++r) {
      const a = this.#e - i + r + 1, l = `${String(a).padStart(n, " ")}: %c`, e = s[r], c = e.length > 75 ? e.slice(0, 75) + "…" : e;
      r === i - 1 ? console.info(
        l + c.slice(0, h.col_s) + "%c" + c.slice(h.col_s),
  ...
  #nt(t) {
    ...
    const h = this.#s.aLNum[this.#t], r = this.#e - ((h ?? 0) || 0), a = this.#s.len;
    for (; this.#t < a; ++this.#t) {
  ...
  #f(t = "", s = "", i = 0) {
    ...
    const l = new B();
    try {
      a = this.#g(t + "@"), l.add({ name: t + ":base", url: n }), l.add({ name: t, url: a });
    } catch {
      l.add({ name: t, url: n });
    }
    return l.use((e, c) => {
...
`) ? a += u.length : a += (u.match(/\n/g) ?? []).length;
  ...
  #mt(t) {
    for (let s = t.len - 1; s >= 0; --s) {
      ...
      const l = n === "loadplugin" ? S.CSS : S.SN, e = this.cfg.matchPath("^" + a.slice(0, -1) + ".*", l);
      for (const c of e) {
        const f = i.replace(
          this.#pt,
  ...
  #bt(t) {
    ...
    for (this.#G += "|" + s, this.#N = new RegExp(`\\[(${this.#G})\\b`), this.hTag[s] = (h) => (h.design_unit = t.design_unit, this.#M(h), this.val.setMp({
      ...h,
      ...
      r === 10 ? this.#e += h.length : r === 91 && (this.#e += (h.match(/\n/g) ?? []).length);
    }
  ...
  loadFromMark(t, s, i = 0) {
    ...
    this.hTag.autowc(h), this.#o = [...this.#u.aIfStk], this.#n = [], x.stopAllTw();
    const r = Promise.allSettled([...n, ...this.#m.playback(this.#u.hPages)]).then(() => this.#m.cover(!1)), { index: a, fn: l } = t;
    if (a)
  ...
  nowScrFnLn() {
    const { fn: t, idx: s } = this.nowScrIdx(), i = this.#a[t], n = this.#c(i, s);
    return { fn: t, ...n };
  }
  //MARK: しおりの保存
  #wt(t) {
  ...
  recodeDesign(t) {
    ...
    if (t.design_unit && n > 0) {
      ...
      s = this.#i, i = this.#t;
    t[":path"] = this.#_(s);
    const h = this.#a[s], r = this.#c(h, i);
    t[":ln"] = r.ln, t[":col_s"] = r.col_s, t[":col_e"] = r.col_e;
    const a = i - 1;
    t[":idx_tkn"] = a, t[":token"] = h.aToken[a], this.sys.send2Dbg("_recodeDesign", t);
  }
  replace(t, s) {
    this.#s.aToken[t] = s;
  }
